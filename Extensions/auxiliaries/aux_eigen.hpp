...
// ILUTCustomPerm extends Eigen::IncompleteLUT
// allows to use a custom permutation
template <typename _Scalar, typename _StorageIndex = int>
class ILUTCustomPerm : public Eigen::IncompleteLUT<_Scalar, _StorageIndex>
{
    public:
        ...
        explicit ILUTCustomPerm(const MatrixType& mat, const std::vector<StorageIndex>& perm, 
            const RealScalar& droptol=Eigen::NumTraits<Scalar>::dummy_precision(), int fillfactor = 10)
        {
            eigen_assert(fillfactor != 0);
            this->m_droptol = droptol;
            this->m_fillfactor = fillfactor;
            this->m_analysisIsOk = false;
            this->m_factorizationIsOk = false;
            compute(mat, perm);
        }

        typename Eigen::IncompleteLUT<Scalar, StorageIndex>::FactorType getLU() { return this->m_lu; }
        PermType getPerm() { return this->m_P; }
        PermType getPermInv() { return this->m_Pinv; }

        template<typename MatrixType>
        void analyzePattern(const MatrixType& amat, const std::vector<StorageIndex>& perm);

        template<typename MatrixType>
        ILUTCustomPerm& compute(const MatrixType& amat, const std::vector<StorageIndex>& perm)
        {
            analyzePattern(amat, perm); 
            this->factorize(amat);
            return *this;
        }
};
...
void ILUTCustomPerm<Scalar,StorageIndex>::analyzePattern(const _MatrixType& amat, const std::vector<StorageIndex>& perm)
{
  assert(amat.rows() == static_cast<Eigen::Index>(perm.size()) && "Permutation size must match matrix size");
  this->m_P.resize(amat.rows());
  for (StorageIndex i : perm)
    this->m_P.indices()(i) = perm[i];
