...
build_tri2dp1_clustergeometry(pctri2dp1 p1, uint * idx)
{
  ...
  for (i = 0; i < triangles; i++) {	/* all triangles */
    ...
    min[0] = H2_REAL_MIN3(x[v[0]][0], x[v[1]][0], x[v[2]][0]);
    min[1] = H2_REAL_MIN3(x[v[0]][1], x[v[1]][1], x[v[2]][1]);
    max[0] = H2_REAL_MAX3(x[v[0]][0], x[v[1]][0], x[v[2]][0]);
    max[1] = H2_REAL_MAX3(x[v[0]][1], x[v[1]][1], x[v[2]][1]);
