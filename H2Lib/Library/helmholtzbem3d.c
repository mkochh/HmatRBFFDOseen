			  ...
			  const real * nx, const real * ny, void *data)
{
  ...
  norm = k_real * norm;
  res = (rnorm * REAL_COS(norm)) + (REAL_SIN(norm) * rnorm) * I_h2;
			  ...
			  const real * nx, const real * ny, void *data)
{
  ...
  s = REAL_SIN(norm);
  c = REAL_COS(norm);
  res = ((c + norm * s) * re + (s - c * norm) * re * I_h2);
			 ...
			 const real * nx, const real * ny, void *data)
{
  ...
  res = rnorm * (c * hr - s * hi + (c * hi + s * hr) * I_h2);
				 ...
				 const void *data)
{
  ...
  res = cexp(I_h2 * k * norm) * rnorm;
  res = res * rnorm * rnorm * (I_h2 * k * norm - 1.0) * REAL_DOT3(n, r);
			       ...
			       const void *data)
{
  ...
  res *= (rnorm * rnorm * (I_h2 * k * norm - 1.0) * REAL_DOT3(n, r)
	  - I_h2 * hdata->eta);
				   ...
				   const void *data)
{
  ...
  res = cexp(I_h2 * REAL_DOT3(kvec, r));
				 ...
				 const void *data)
{
  ...
  res = I_h2 * REAL_DOT3(kvec, n) * cexp(I_h2 * REAL_DOT3(kvec, r));
			       ...
			       const void *data)
{
  ...
  res = I_h2 * (REAL_DOT3(kvec, n) - hdata->eta) * cexp(I_h2 * REAL_DOT3(kvec, r));
