...
build_bem2d_const_clustergeometry(pcbem2d bem, uint ** idx)
{
  ...
  *idx = (uint *) allocmem(edges * sizeof(uint));
  ...
  for (i = 0; i < edges; i++) {
    ...
    cg->smin[i][0] = H2_REAL_MIN(x[e[i][0]][0], x[e[i][1]][0]);
    cg->smin[i][1] = H2_REAL_MIN(x[e[i][0]][1], x[e[i][1]][1]);

    /* Upper back right corner of bounding box */
    cg->smax[i][0] = H2_REAL_MAX(x[e[i][0]][0], x[e[i][1]][0]);
    cg->smax[i][1] = H2_REAL_MAX(x[e[i][0]][1], x[e[i][1]][1]);
  }
  return cg;
}
...
build_bem2d_linear_clustergeometry(pcbem2d bem, uint ** idx)
{
  ...
  *idx = (uint *) allocmem(vertices * sizeof(uint));
  ...
  for (i = 0; i < edges; i++) {
    ...
    tmin[0] = H2_REAL_MIN(x[tv0][0], x[tv1][0]);
    tmin[1] = H2_REAL_MIN(x[tv0][1], x[tv1][1]);

    /* Upper back right corner of bounding box for current triangle */
    tmax[0] = H2_REAL_MAX(x[tv0][0], x[tv1][0]);
    tmax[1] = H2_REAL_MAX(x[tv0][1], x[tv1][1]);
