...
build_tet3dp1_clustergeometry(pctet3dp1 p1, uint * idx)
{
  ...
  for (i = 0; i < tetrahedra; i++) {	/* all tetrahedra */
    ...
    min[0] = H2_REAL_MIN(H2_REAL_MIN(x[v[0]][0], x[v[1]][0]),
		      H2_REAL_MIN(x[v[2]][0], x[v[3]][0]));
    min[1] = H2_REAL_MIN(H2_REAL_MIN(x[v[0]][1], x[v[1]][1]),
		      H2_REAL_MIN(x[v[2]][1], x[v[3]][1]));
    min[2] = H2_REAL_MIN(H2_REAL_MIN(x[v[0]][2], x[v[1]][2]),
		      H2_REAL_MIN(x[v[2]][2], x[v[3]][2]));
    max[0] = H2_REAL_MAX(H2_REAL_MAX(x[v[0]][0], x[v[1]][0]),
		      H2_REAL_MAX(x[v[2]][0], x[v[3]][0]));
    max[1] = H2_REAL_MAX(H2_REAL_MAX(x[v[0]][1], x[v[1]][1]),
		      H2_REAL_MAX(x[v[2]][1], x[v[3]][1]));
    max[2] = H2_REAL_MAX(H2_REAL_MAX(x[v[0]][2], x[v[1]][2]),
		      H2_REAL_MAX(x[v[2]][2], x[v[3]][2]));

    for (j = 0; j < 4; j++) {	/* all vertices */
      if (is_dof[v[j]]) {	/* vertex is degree of freedom */
	k = idx2dof[v[j]];

	cg->smin[k][0] = H2_REAL_MIN(cg->smin[k][0], min[0]);
	cg->smin[k][1] = H2_REAL_MIN(cg->smin[k][1], min[1]);
	cg->smin[k][2] = H2_REAL_MIN(cg->smin[k][2], min[2]);
	cg->smax[k][0] = H2_REAL_MAX(cg->smax[k][0], max[0]);
	cg->smax[k][1] = H2_REAL_MAX(cg->smax[k][1], max[1]);
	cg->smax[k][2] = H2_REAL_MAX(cg->smax[k][2], max[2]);
      }
    }
  }
