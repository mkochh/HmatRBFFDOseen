...
fill_slp_cc_sing_cpu_wrapper_helmholtzbem3d(void *data)
{
  ...
  for (t = 0; t < pos; ++t) {
	...
	sum += wq[q] * (cos(norm) + I_h2 * sin(norm)) * rnorm;
      }
      else {
	norm = norm2 * rnorm;
	sum += wq[q] * exp(-IMAG(k) * norm)
	  * (cos(REAL(k) * norm) + I_h2 * sin(REAL(k) * norm)) * rnorm;
      }
...
fill_dlp_cc_sing_cpu_wrapper_helmholtzbem3d(void *data)
{
  ...
  for (t = 0; t < pos; ++t) {
	...
	if (IMAG(k) == 0.0) {
	  norm = REAL(k) * norm2 * rnorm;
	  sum += wq[q] * (cos(norm) + I_h2 * sin(norm)) * rnorm * rnorm * rnorm
	    * (1.0 - I_h2 * norm) * (dx * ns[0] + dy * ns[1] + dz * ns[2]);
	}
	else {
	  norm = norm2 * rnorm;
	  sum += wq[q] * exp(-IMAG(k) * norm)
	    * (cos(REAL(k) * norm) + I_h2 * sin(REAL(k) * norm)) * rnorm
	    * rnorm * rnorm * (1.0 - I_h2 * k * norm)
