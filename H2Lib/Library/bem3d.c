...
build_bem3d_const_clustergeometry(pcbem3d bem, uint ** idx)
{
  ...
  *idx = allocuint(triangles);
  ...
  for (i = 0; i < triangles; i++) {
    ...
    cg->smin[i][0] = H2_REAL_MIN3(x[t[i][0]][0], x[t[i][1]][0], x[t[i][2]][0]);
    cg->smin[i][1] = H2_REAL_MIN3(x[t[i][0]][1], x[t[i][1]][1], x[t[i][2]][1]);
    cg->smin[i][2] = H2_REAL_MIN3(x[t[i][0]][2], x[t[i][1]][2], x[t[i][2]][2]);

    /* Upper back right corner of bounding box */
    cg->smax[i][0] = H2_REAL_MAX3(x[t[i][0]][0], x[t[i][1]][0], x[t[i][2]][0]);
    cg->smax[i][1] = H2_REAL_MAX3(x[t[i][0]][1], x[t[i][1]][1], x[t[i][2]][1]);
    cg->smax[i][2] = H2_REAL_MAX3(x[t[i][0]][2], x[t[i][1]][2], x[t[i][2]][2]);
...
build_bem3d_linear_clustergeometry(pcbem3d bem, uint ** idx)
{
  ...
  *idx = allocuint(vertices);
  ...
  for (i = 0; i < triangles; i++) {
    ...
    tmin[0] = H2_REAL_MIN3(x[tv0][0], x[tv1][0], x[tv2][0]);
    tmin[1] = H2_REAL_MIN3(x[tv0][1], x[tv1][1], x[tv2][1]);
    tmin[2] = H2_REAL_MIN3(x[tv0][2], x[tv1][2], x[tv2][2]);

    /* Upper back right corner of bounding box for current triangle */
    tmax[0] = H2_REAL_MAX3(x[tv0][0], x[tv1][0], x[tv2][0]);
    tmax[1] = H2_REAL_MAX3(x[tv0][1], x[tv1][1], x[tv2][1]);
    tmax[2] = H2_REAL_MAX3(x[tv0][2], x[tv1][2], x[tv2][2]);
    ...
    for (j = 0; j < 3; ++j) {
      ...
      /* Lower front left corner of bounding box */
      cg->smin[tv0][0] = H2_REAL_MIN(cg->smin[tv0][0], tmin[0]);
      cg->smin[tv0][1] = H2_REAL_MIN(cg->smin[tv0][1], tmin[1]);
      cg->smin[tv0][2] = H2_REAL_MIN(cg->smin[tv0][2], tmin[2]);

      /* Upper back right corner of bounding box */
      cg->smax[tv0][0] = H2_REAL_MAX(cg->smax[tv0][0], tmax[0]);
      cg->smax[tv0][1] = H2_REAL_MAX(cg->smax[tv0][1], tmax[1]);
      cg->smax[tv0][2] = H2_REAL_MAX(cg->smax[tv0][2], tmax[2]);
    }
  }
		      ...
		      pamatrix V, kernel_simd_func3d kernel)
{
  ...
  for (t = 0; t < rows; ++t) {
	...
	sum_i = vfmadd(w, eval_i, sum_i);
      ...
      sum =
	((vreduce(sum_r) + base) * gt_fac) + (vreduce(sum_i) * gt_fac) * I_h2;
		      ...
		      pamatrix V, kernel_simd_func3d kernel)
{
  ...
  for (s = 0; s < rows; ++s) {
	...
	sum_i = vfmadd(w, eval_i, sum_i);
      ...
      sum =
	((vreduce(sum_r) + base) * gs_fac) + (vreduce(sum_i) * gs_fac) * I_h2;
				       ...
				       pcreal dir, pcbem3d bem, pamatrix V)
{
  ...
  for (jx = 0; jx < mx; ++jx) {
	...
	for (t = 0; t < rows; ++t) {
	  ...
	  V->a[t + index * ld] = (gt * sum_r) + (gt * sum_i) * I_h2;
	}
	index++;
      }
    }
	...
	for (jz = 0; jz < mz; ++jz) {
	  ...
	  while (vl) {
		...
		if (ii == tri_sp[i]) {
		  ...
		  aa[k + index * ld] += (sum_r * gs) + (sum_i * gs) * I_h2;
		}
		ww += vnq;
	      }
	      ww = bem->sq->w_single;
